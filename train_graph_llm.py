...
def main(args, SEED):
    ...
    # Create a tensor of numbers from 0 to 275
    data = torch.arange(275)
    ...
    with accelerator.main_process_first():
        ...
        clm_dataset_train = dataset.map(
            preprocess_train_dataset[args.dataset](tokenizer=tokenizer,prediction_type=args.prediction_type, max_length=instruction_len[args.dataset]),
            batched=True,
            batch_size=None,
            remove_columns=[i for i in dataset.column_names if i not in ['node_ids']],
            keep_in_memory=True,
        ...
        clm_dataset_test = dataset.map(
            preprocess_test_dataset[args.dataset](tokenizer=tokenizer, max_length=instruction_len[args.dataset],prediction_type=args.prediction_type),
            batched=True,
            batch_size=None,
    ...
    ckpt = "/home/sphaire/gnn_llm/trial/Graph-LLM/Meta-Llama-3.1-8B/consolidated.00.pth"
    ...
    for epoch in range(args.num_epochs):
        ...
        accelerator.print(f"Epoch: {epoch}|{args.num_epochs}: Train Loss (Epoch Mean): {epoch_loss / len(train_loader)}")
        ...
        if epoch % 20 == 0:
            print(f'eval pred is {eval_pred}')
            print(f'eval label is {eval_label}')
        ...
        if val_acc > best_val_acc:
            ...
            accelerator.save_state(f"/home/sphaire/gnn_llm/trial/Graph-LLM/fine-tuned-gllama_{args.prediction_type}_10")
            best_epoch = epoch
            model = model.cuda()

        accelerator.print(f'Epoch {epoch} Val Acc {val_acc} Best Val Acc {best_val_acc}')
